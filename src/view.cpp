...
int View::getFd() const
{
	return m_fd;
}

unsigned View::getWidth() const
{
	return m_width * m_zoom;
}

double View::getSpeed() const
{
	return m_speed;
}
...
void View::updateMouse(unsigned x, unsigned /* y */)
{
	m_mousex = x;
	updateStatus();
	redrawStatus();
...
void View::drawStatusChar(unsigned x, char ch)
{
...
void View::clearSignal()
{
	memset(m_res.mainImage->data, 0x00, m_width * m_signalHeight * 4);
}

void View::drawSignalLine(unsigned x1, unsigned y1, unsigned y2)
{
...
void View::drawSignalVertStripe(unsigned x, unsigned ystart)
{
...
void View::drawSignalPixel(unsigned x, unsigned y, const uint8_t bgr[])
{
	const unsigned ofs(((y * m_width) + x) * 4);
	memcpy(m_res.mainImage->data + ofs, bgr, 3);
}

double View::getMaxSpeed() const
{
